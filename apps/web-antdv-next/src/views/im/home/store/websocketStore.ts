  ...
  actions: {
    ...
    connect() {
      ...
      const onClose = () => {
        ...
        console.warn('[IM WS] disconnected');
        this.reconnect();
      };
    ...
    dispatchFrame(frame: WebSocketFrame) {
      if (frame.type !== ImWebSocketMessageType.NOTIFICATION) {
        console.warn('[IM WS] 未识别事件', frame);
        return;
      }
      ...
      switch (notification.conversationType) {
        ...
        default: {
          console.warn('[IM WS] 未识别通知', notification);
        }
      }
    },
    ...
    dispatchNoConversationFrame(
      ...
      switch (websocketMessage.type) {
        ...
        default: {
          console.warn('[IM WS] 未识别无会话通知', websocketMessage);
        }
      }
    },
    ...
    handleFriendNotification(
      ...
      switch (websocketMessage.type) {
        ...
        default: {
          console.warn('[IM WS] 未识别好友通知', websocketMessage);
        }
      }
    },
    ...
    handleGroupMemberSettingUpdate(
      ...
      if (payload.groupRemark !== undefined && payload.groupRemark !== null) {
    ...
    handleRtcCallEnd(
      ...
      if (rtcStore.isActive && (matchCall || matchIncoming)) {
        ...
        console.warn('[Call] end:', reasonText);
        rtcStore.reset();
      }
    },
  },
