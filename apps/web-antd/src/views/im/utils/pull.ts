...
export async function runIncrementalPull<T extends PullRecord>(
  ...
  for (let page = 0; page < PULL_MAX_PAGES; page++) {
    ...
    if (list.length > 0) {
      ...
      if (last.updateTime === undefined || last.updateTime === null) {
        return;
      }
      const lastUpdateTime = last.updateTime;
      const lastId = last.id;
      cursor = { lastUpdateTime, lastId };
      if (
        highWater.lastUpdateTime === undefined ||
        highWater.lastUpdateTime === null ||
        lastUpdateTime > highWater.lastUpdateTime ||
        (lastUpdateTime === highWater.lastUpdateTime &&
          lastId > (highWater.lastId ?? 0))
      ) {
        highWater.lastUpdateTime = lastUpdateTime;
        highWater.lastId = lastId;
...
export async function runMinIdPull<T extends { id?: number }>(options: {
  applyPage: (records: T[], nextMinId?: number) => Promise<boolean | undefined>;
  ...
  for (let page = 0; page < maxPages; page++) {
    ...
    if (nextMinId === undefined || nextMinId === null || nextMinId <= minId) {
      return;
    }
    minId = nextMinId;
  }
