...
export function enqueueConversationWrites<T>(
  ...
  const keys = [...new Set(clientConversationIds)].toSorted();
  const predecessors = [
    writeState.barrierTail,
    ...keys.map((key) => writeState.tails.get(key) || Promise.resolve()),
  ];
  const current = Promise.all(
    predecessors.map((task) => task.catch(() => undefined)),
  ).then(operation);
...
export function enqueueConversationBarrier<T>(
  ...
  const existingWrites = [...writeState.tails.values()];
  let release!: () => void;
  const gate = new Promise<void>((resolve) => {
    release = resolve;
  });
  writeState.barrierTail = previousBarrier
    .catch(() => undefined)
    .then(() => gate);
  return (async () => {
    ...
    await Promise.all(
      existingWrites.map((task) => task.catch(() => undefined)),
    );
...
export function markRelationTerminated(
  clientConversationId: string,
  messageId?: number,
): boolean {
...
export function reopenRelation(
  clientConversationId: string,
  messageId?: number,
): boolean {
...
export async function clearMessageSyncState(
  shouldClear: () => boolean,
): Promise<void> {
  const barrier = writeState.barrierTail;
  const tails = [...writeState.tails.entries()];
  await Promise.all([
