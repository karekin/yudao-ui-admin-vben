...
import { getClientConversationId, getDb, initDb } from '../../utils/db';
import {
...
function isSelfInPayloadMembers(
...
export const useGroupStore = defineStore('imGroupStore', {
  ...
  actions: {
    ...
    saveGroup(group: Group | undefined, db: DbClient = getDb()): void {
      ...
      ).catch((error) => console.warn('[IM groupStore] 本地群写入失败', error));
    },
    ...
    async loadGroupMemberList(groupId: number): Promise<GroupMember[] | null> {
      ...
      try {
        ...
        return await enqueueConversationWrite(
    ...
    async fetchGroupList(force = false) {
      ...
      return runResourceRequest(
        ...
        async () => {
          ...
          const committed = await enqueueConversationWrites(
            conversationIds,
            async () => {
              ...
              await this.saveGroupList([...this.groups], db).catch((error) =>
    ...
    fetchGroupInfo(
      ...
      const promise = (async () => {
        ...
        const shouldRefresh =
          queuedGroupInfoRefreshes.has(key) && isRelationCurrent();
      ...
      const promise = (async () => {
        ...
        return await enqueueConversationWrite(
          ...
          async () => {
            ...
            const group = this.getGroup(groupId);
            const isPlaceholder = group === undefined;
        ...
        if (pendingMemberFetches.get(key) === promise) {
          ...
          if (shouldRefresh) {
            void this.fetchGroupMemberList(groupId, true, db).catch(
              () => undefined,
            );
          }
        }
    ...
    fetchGroupMember(
      ...
      const key = pendingSingleMemberKey(
        groupId,
        relationVersion,
        memberUserId,
      );
    ...
    async removeGroupNow(
      ...
      if (
        hiddenConversation &&
        conversationStore.getConversation(ImConversationType.GROUP, id) ===
          currentConversation
      ) {
        conversationStore.publishHiddenConversationProjection(
          hiddenConversation,
        );
      }
    },
    ...
    async applyGroupNotificationNow(
      ...
      switch (type) {
        ...
        case ImContentType.GROUP_CREATE: {
          await this.applyGroupCreateNotificationNow(
            groupId,
            payload,
            messageId,
            db,
        ...
        case ImContentType.GROUP_MEMBER_INVITE: {
          await this.applyGroupMemberInviteNotificationNow(
            groupId,
            payload,
            messageId,
            db,
        ...
        case ImContentType.GROUP_MEMBER_KICK: {
          await this.applyGroupMemberKickNotificationNow(
            groupId,
            payload,
            messageId,
            db,
        ...
        case ImContentType.GROUP_MEMBER_QUIT: {
          await this.applyGroupMemberQuitNotificationNow(
            groupId,
            payload,
            messageId,
            db,
        ...
        case ImContentType.GROUP_OWNER_TRANSFER: {
          this.applyGroupOwnerTransferNotification(
            groupId,
            payload,
            db.userId,
            db,
    ...
    async reopenGroupRelationNow(
      ...
      await useConversationStore().saveConversationRecord(
        nextConversation,
        undefined,
        db,
      );
      useConversationStore().publishConversationProjection(
        nextConversation,
        true,
      );
      return true;
    ...
    applyGroupInfoUpdateNotification(
      ...
      if (
        payload.newJoinApproval !== undefined &&
        payload.newJoinApproval !== null
      ) {
    ...
    async applyGroupMemberInviteNotificationNow(
      ...
      if (
        selfJoined &&
        !(await this.reopenGroupRelationNow(groupId, messageId, db))
      ) {
        return;
      }
