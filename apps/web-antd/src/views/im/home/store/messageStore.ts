...
/** 获取普通消息与撤回终态的归约优先级 */
function getMessageTerminalPriority(
  message: Pick<Message, 'id' | 'status' | 'type'>,
) {
  if (
...
function isSameMessageDOPageCursor(
  ...
  return (
    left.sendTime === right.sendTime && left.messageKey === right.messageKey
  );
}
...
function shouldUpdateConversationSummary(
  ...
  return (
    (!!message.clientMessageId &&
      message.clientMessageId === conversation.lastClientMessageId) ||
...
/** 构建服务端消息更新后的下一份投影 */
function buildServerMessageProjection(
  message: Message,
...
export const useMessageStore = defineStore('imMessageStore', {
  ...
  actions: {
    ...
    touchConversationMessageCache(clientConversationId: string) {
      ...
      removed.forEach((key) => {
        const { [key]: _removedMessages, ...nextMessagesByConversation } =
          this.messagesByConversation;
        const { [key]: _removedCursor, ...nextMessageDOPageCursors } =
          this.messageDOPageCursors;
        this.messagesByConversation = nextMessagesByConversation;
        this.messageDOPageCursors = nextMessageDOPageCursors;
      });
    },
    ...
    async loadMoreMessageListNow(
      ...
      const terminal = await getConversationMessageTerminal(
        clientConversationId,
        db,
      );
      if (
        !before &&
        !this.recoveredConversationKeys.includes(clientConversationId)
      ) {
        await this.recoverPendingMessageListNow(
          clientConversationId,
          terminal,
          db,
        );
      ...
      await db.transaction(['messages'], 'readwrite', async (tx) => {
        ...
        for (const record of records) {
          ...
          if (recovered !== record || recalled !== recoveredMessage) {
            await db.put(
              'messages',
              buildMessageDO(recalled, record.conversationType),
              tx,
            );
          }
        }
    ...
    async ensureConversationMessageListLoaded(conversation: Conversation) {
      const key = getClientConversationId(
        conversation.type,
        conversation.targetId,
      );
      if (
        this.messagesByConversation[key] &&
        this.recoveredConversationKeys.includes(key)
      ) {
    ...
    async applyRecallMessageRecord(
      ...
      const recalledKeys =
        (await db.getSetting<string[]>(recallSettingKey, tx)) || [];
      await db.setSetting(
        recallSettingKey,
        [...new Set([...recalledKeys, `id:${messageId}`])],
        tx,
      );
      const conversationStore = useConversationStore();
      const currentConversation =
        staged?.conversation ||
        conversationStore.getConversation(conversationType, targetId);
    ...
    async applyPulledMessageListNow(
      ...
      for (const pulledMessage of pulledMessages) {
        ...
        if (!projection) {
          ...
          projection = {
            conversation:
              conversationStore.buildConversationProjection(conversationInfo),
            currentMessages,
        ...
        const isUnread =
          ...
          message.status !== ImMessageStatus.RECALL;
        let existingIndex = messages.findIndex((existing) =>
        ...
        if (existingIndex < 0 && message.id) {
          ...
          if (storedMessage) {
            existingIndex = messages.findIndex(
              (existing) => existing.id && messageId < existing.id,
            );
            if (existingIndex < 0) {
            ...
            messages.splice(
              existingIndex,
              0,
              buildMessageFromDO(storedMessage),
            );
          }
        }
        if (existingIndex >= 0) {
          ...
          if (reduced.value === message) {
            messages[existingIndex] = buildServerMessageProjection(
              existing,
              message,
            );
          }
        ...
        async (tx) => {
          ...
          for (const recallMessage of recallMessages) {
            ...
            if (changed) {
              ...
              let projection =
                conversationProjections.get(clientConversationId);
              if (projection) {
      ...
      for (const [
        clientConversationId,
        projection,
      ] of conversationProjections) {
        for (const current of projection.currentMessages) {
          const next = projection.nextMessages.find((message) =>
            isSameMessage(message, current),
          );
        ...
        this.messagesByConversation[clientConversationId] =
          projection.nextMessages;
        this.touchConversationMessageCache(clientConversationId);
        conversationStore.publishConversationProjection(
    ...
    async insertMessageNow(
      ...
      let existingIndex = messages.findIndex((item) =>
      ...
      if (existingIndex >= 0) {
        ...
        const currentMessage = currentMessages.find((item) =>
          isSameMessage(item, message),
        );
        if (
          currentMessage &&
          currentMessage.content !== messages[existingIndex]!.content
        ) {
    ...
    async doAckMessageNow(
      ...
      const currentConversation = conversationStore.getConversation(
        conversationType,
        targetId,
      );
      ...
      try {
        ...
        if (reduced.value === incoming) {
          ...
          nextMessage = buildServerMessageProjection(
            message,
            nonTerminalUpdates,
          );
        }
    ...
    async applyMessageReadReceiptNow(
      ...
      if (
        ...
        const storedByKey = new Map(
          storedMessages.map((message) => [message.messageKey, message]),
        );
        storedMessages
          .filter(
            (message) =>
        ...
        async (tx) => {
          ...
          const deletedKeys = [
            ...
            ...(removed.clientMessageId
              ? [`client:${removed.clientMessageId}`]
              : []),
          ];
          await db.setSetting(
            settingKey,
            [...new Set([...oldKeys, ...deletedKeys])],
            tx,
          );
          await db.delete(
            'messages',
            getMessageKey(removed, conversationType),
            tx,
          );
          if (index === nextMessages.length) {
            if (nextMessages.length === 0) {
              ...
              const latest = storedMessages
                .toSorted(compareConversationSummaryOrder)
                .at(-1);
    ...
    async deleteConversationMessageListNow(
      ...
      const messages = this.messagesByConversation[clientConversationId] || [];
      const conversation = useConversationStore().getConversation(
        conversationType,
        targetId,
      );
      ...
      await db.transaction(
        ...
        async (tx) => {
          ...
          const oldClearBefore =
            (await db.getSetting<number>(settingKey, tx)) || 0;
          const deletedSettingKey = `${StorageKeys.settings.conversationDeletedMessagesPrefix}${clientConversationId}`;
          const oldDeletedKeys =
            (await db.getSetting<string[]>(deletedSettingKey, tx)) || [];
          ...
          if (pendingClientKeys.size > 0) {
            await db.setSetting(
              deletedSettingKey,
              [...new Set([...oldDeletedKeys, ...pendingClientKeys])],
              tx,
            );
          }
          await db.deleteByIndex(
            'messages',
            'clientConversationId',
            clientConversationId,
            tx,
      ...
      const {
        [clientConversationId]: _removedMessages,
        ...nextMessagesByConversation
      } = this.messagesByConversation;
      const {
        [clientConversationId]: _removedCursor,
        ...nextMessageDOPageCursors
      } = this.messageDOPageCursors;
      this.messagesByConversation = nextMessagesByConversation;
      this.messageDOPageCursors = nextMessageDOPageCursors;
