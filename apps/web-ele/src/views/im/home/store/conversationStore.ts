import type { DbClient, DbTransaction } from '../../utils/db';
import type {
  Conversation,
  ConversationDO,
  ConversationRead,
...
function isValidConversationReadRecord(
  record: ImConversationReadRespVO,
): boolean {
...
function applyConversationUnreadState(
  ...
  for (const message of messages) {
    ...
    unreadCount++;
    if (
      message.atUserIds?.includes(userId) &&
...
function applyConversationRecallStateWithoutRead(
  ...
  if (
    ...
    (!conversation.atMessageId &&
      conversation.atMe &&
      originalMessage.atUserIds?.includes(userId))
  ) {
...
function backfillConversationMentionIds(
  ...
  const atMessageId =
    conversation.atMessageId ||
    (conversation.atMe
      ? unreadMessages.find((message) => message.atUserIds?.includes(userId))
          ?.id
      : undefined);
...
export const useConversationStore = defineStore('imConversationStore', {
  ...
  actions: {
    ...
    async saveConversationReadRecord(
      ...
      let readRecords: ConversationRead[] = [];
      if (Array.isArray(target)) {
        readRecords = target;
      } else if (target) {
        readRecords = [target];
      }
      const records = readRecords.map((item) => toConversationReadDO(item));
      if (records.length === 0) {
        return;
      }
      if (tx) {
    ...
    /** 应用本地会话读位置 */
    async applyLocalConversationReads(
    ...
    async applyConversationReadList(
      ...
      const conversationIds = records
        .filter((record) => isValidConversationReadRecord(record))
        .map((record) =>
          getClientConversationId(record.conversationType, record.targetId),
        );
    ...
    async applyConversationReadListNow(
      ...
      for (const record of records) {
        ...
        const current =
          changedReads.get(clientConversationId) ||
          this.conversationReads[clientConversationId];
      ...
      if (stores.length > 0) {
        await db.transaction(stores, 'readwrite', async (tx) => {
          if (changedReads.size > 0) {
            await this.saveConversationReadRecord(
              [...changedReads.values()],
              tx,
              db,
            );
      ...
      changedMemoryMessages.forEach((next, current) =>
    ...
    async saveConversationRecord(
      ...
      const conversations = records.map((item) => toConversationDO(item));
    ...
    saveConversation(
      ...
      if (tx) {
        void this.saveConversationRecord(conversation, tx, db).catch((error) =>
    ...
    saveConversationList(
      ...
      if (tx) {
        return this.saveConversationRecord(targets, tx, db).catch((error) =>
          console.warn('[IM conversationStore] 会话列表写入失败', error),
        );
      }
      return enqueueConversationWrites(
        targets.map((item) =>
          getClientConversationId(item.type, item.targetId),
        ),
    ...
    buildConversationProjection(info: {
      ...
      const relationTerminated =
        info.type === ImConversationType.GROUP &&
      ...
      if (current) {
        ...
        if (preserveConcurrentFields) {
          if (!base || current.name !== base.name)
            concurrentFields.name = current.name;
          if (!base || current.avatar !== base.avatar)
            concurrentFields.avatar = current.avatar;
          if (!base || current.top !== base.top)
            concurrentFields.top = current.top;
          if (!base || current.silent !== base.silent)
            concurrentFields.silent = current.silent;
          if (!base || current.draft !== base.draft)
            concurrentFields.draft = current.draft;
        }
    ...
    ensureConversation(info: {
      ...
      return this.publishConversationProjection(
        this.buildConversationProjection(info),
      );
    },
    ...
    setActiveConversation(conversation: Conversation | null) {
      ...
      void useMessageStore()
        ...
        .catch((error) =>
          console.warn('[IM conversationStore] 会话消息加载失败', error),
        );
      this.saveConversation(conversation);
    },
    ...
    async saveHiddenConversationRecord(
      ...
      const stored = current
        ? undefined
        : await db.get<ConversationDO>(
            'conversations',
            clientConversationId,
            tx,
          );
      const conversation =
        current || (stored ? fromConversationDO(stored) : undefined);
      if (!conversation) {
        return undefined;
      }
    ...
    async hideConversationNow(type: number, targetId: number, db: DbClient) {
      const projection = await this.saveHiddenConversationRecord(
        type,
        targetId,
        undefined,
        db,
    ...
    markConversationRead(
      ...
      void enqueueConversationWrite(
        ...
        console.warn('[IM conversationStore] 会话已读写入失败', error),
      );
    },
    ...
    markConversationReadReported(
      ...
      void enqueueConversationWrite(
        ...
        console.warn('[IM conversationStore] 已上报读位置写入失败', error),
      );
    },
...
const saveDraftConversationListDebounced = debounce(async (): Promise<void> => {
  const conversations = [...pendingDraftConversations.entries()];
  pendingDraftConversations.clear();
  if (conversations.length === 0) {
    return;
  }
